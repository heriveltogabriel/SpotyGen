...
// -------------------------------------------------------------
// GERENCIADOR DE EDIÇÃO COMPLETA DA COLEÇÃO (LP DA SEMANA)
// -------------------------------------------------------------

function openCollectionEditor() {
    if (!state.accessToken) {
        showToast('Conecte-se ao Spotify no PlaylistGen para acessar o Gerenciador do LP da Semana.', 'error');
        redirectToSpotifyAuth();
        return;
    }
    if (DOM.collectionEditorModal) {
        DOM.collectionEditorModal.classList.remove('hidden');
...
function closeCollectionEditor() {
    if (DOM.collectionEditorModal) {
        DOM.collectionEditorModal.classList.add('hidden');
    }
    state.collectionItems = [];
    state.collectionExpandedIndex = null;
}

async function loadCollectionForEditing() {
    if (!DOM.collectionListContainer) return;
    DOM.collectionListContainer.innerHTML = '<div style="text-align:center; padding: 2rem; color: var(--text-muted);">Carregando coleção de LPs...</div>';
    try {
        const res = await fetch('/api/playlists');
        if (!res.ok) throw new Error('Erro ao carregar playlists');
        const data = await res.json();
        state.collectionItems = data;
        state.collectionExpandedIndex = null;
        renderCollectionEditor();
    } catch (e) {
        console.error(e);
        DOM.collectionListContainer.innerHTML = `<div style="text-align:center; padding: 2rem; color: #e53e3e;">Erro ao carregar coleção: ${e.message}</div>`;
    }
}

function renderCollectionEditor(filterText = '') {
    const container = DOM.collectionListContainer;
    if (!container) return;

    const filter = filterText.toLowerCase().trim();
    
    let filtered = state.collectionItems.map((item, originalIndex) => ({ item, originalIndex }));
    if (filter) {
    ...
    if (DOM.collectionCount) DOM.collectionCount.textContent = state.collectionItems.length;

    if (filtered.length === 0) {
        container.innerHTML = filter ? 
            '<div style="text-align:center; padding: 2rem; color: var(--text-muted);">Nenhum LP encontrado para o filtro digitado.</div>' :
    ...
    filtered.forEach(({ item, originalIndex }) => {
        const isExpanded = state.collectionExpandedIndex === originalIndex;
        const card = document.createElement('div');
        card.className = 'collection-item-card';

        card.innerHTML = `
            <div class="collection-item-header">
                <div class="collection-item-info" data-index="${originalIndex}">
                    <img class="collection-item-thumb" src="${item.imageUrl || 'logo.jpg'}" alt="Capa" onerror="this.src='logo.jpg'">
                    <div class="collection-item-titles">
                        <span class="collection-item-title">${escapeHtml(item.name || 'Sem nome')}</span>
                        <span class="collection-item-sub">${escapeHtml(item.description || 'LP da Semana')}</span>
                    </div>
                </div>
                <div class="collection-item-actions">
                    <button class="btn-icon-sm btn-move-up" data-index="${originalIndex}" title="Mover para cima" ${originalIndex === 0 ? 'disabled style="opacity:0.3;"' : ''}>
                        <i data-lucide="chevron-up"></i>
                    </button>
                    <button class="btn-icon-sm btn-move-down" data-index="${originalIndex}" title="Mover para baixo" ${originalIndex === state.collectionItems.length - 1 ? 'disabled style="opacity:0.3;"' : ''}>
                        <i data-lucide="chevron-down"></i>
                    </button>
                    <button class="btn-icon-sm btn-toggle-edit" data-index="${originalIndex}" title="Editar campos">
                        <i data-lucide="${isExpanded ? 'chevron-up' : 'edit-2'}"></i>
                    </button>
                    <button class="btn-icon-sm btn-icon-danger btn-delete-item" data-index="${originalIndex}" title="Excluir este LP">
            ...
            <div class="collection-item-form">
                <div class="form-group-full">
                    <label>Título (Formato: Artista - Álbum)</label>
                    <input type="text" class="edit-field" data-index="${originalIndex}" data-field="name" value="${escapeHtml(item.name || '')}">
                </div>
                <div class="form-group-full">
                    <label>Descrição / Número do LP (Ex: LP da Semana #35)</label>
                    <input type="text" class="edit-field" data-index="${originalIndex}" data-field="description" value="${escapeHtml(item.description || '')}">
                </div>
                <div>
                    <label>Spotify URL</label>
                    <input type="text" class="edit-field" data-index="${originalIndex}" data-field="spotifyUrl" value="${escapeHtml(item.spotifyUrl || '')}">
                </div>
                <div>
                    <label>YouTube Music URL</label>
                    <input type="text" class="edit-field" data-index="${originalIndex}" data-field="youtubeUrl" value="${escapeHtml(item.youtubeUrl || '')}">
                </div>
                <div>
                    <label>Apple Music URL</label>
                    <input type="text" class="edit-field" data-index="${originalIndex}" data-field="appleMusicUrl" value="${escapeHtml(item.appleMusicUrl || '')}">
                </div>
                <div>
                    <label>Deezer URL</label>
                    <input type="text" class="edit-field" data-index="${originalIndex}" data-field="deezerUrl" value="${escapeHtml(item.deezerUrl || '')}">
                </div>
                <div>
                    <label>Amazon Music URL</label>
                    <input type="text" class="edit-field" data-index="${originalIndex}" data-field="amazonMusicUrl" value="${escapeHtml(item.amazonMusicUrl || '')}">
                </div>
                <div>
                    <label>Image Cover URL</label>
                    <input type="text" class="edit-field" data-index="${originalIndex}" data-field="imageUrl" value="${escapeHtml(item.imageUrl || '')}">
                </div>
            </div>
            ` : ''}
        `;
    ...
    // Eventos internos nos cards
    container.querySelectorAll('.collection-item-info, .btn-toggle-edit').forEach(el => {
        el.addEventListener('click', (e) => {
            e.stopPropagation();
            const idx = parseInt(el.getAttribute('data-index'));
            state.collectionExpandedIndex = (state.collectionExpandedIndex === idx) ? null : idx;
            renderCollectionEditor(DOM.collectionSearchInput ? DOM.collectionSearchInput.value : '');
        });
    });
    ...
    container.querySelectorAll('.btn-move-up').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (idx > 0) {
                const temp = state.collectionItems[idx];
                state.collectionItems[idx] = state.collectionItems[idx - 1];
                state.collectionItems[idx - 1] = temp;
                if (state.collectionExpandedIndex === idx) state.collectionExpandedIndex = idx - 1;
                else if (state.collectionExpandedIndex === idx - 1) state.collectionExpandedIndex = idx;
                renderCollectionEditor(DOM.collectionSearchInput ? DOM.collectionSearchInput.value : '');
            }
        });
    });
    ...
    container.querySelectorAll('.btn-move-down').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const idx = parseInt(btn.getAttribute('data-index'));
            if (idx < state.collectionItems.length - 1) {
                const temp = state.collectionItems[idx];
                state.collectionItems[idx] = state.collectionItems[idx + 1];
                state.collectionItems[idx + 1] = temp;
                if (state.collectionExpandedIndex === idx) state.collectionExpandedIndex = idx + 1;
    ...
    container.querySelectorAll('.btn-delete-item').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const idx = parseInt(btn.getAttribute('data-index'));
            const item = state.collectionItems[idx];
            if (confirm(`Tem certeza que deseja remover o LP "${item.name}" da coleção?`)) {
                state.collectionItems.splice(idx, 1);
                if (state.collectionExpandedIndex === idx) state.collectionExpandedIndex = null;
                else if (state.collectionExpandedIndex > idx) state.collectionExpandedIndex--;
                renderCollectionEditor(DOM.collectionSearchInput ? DOM.collectionSearchInput.value : '');
                showToast(`LP "${item.name}" removido da lista local. Clique em Salvar para publicar.`, 'success');
            }
        });
    });

    container.querySelectorAll('.edit-field').forEach(input => {
        input.addEventListener('input', () => {
            const idx = parseInt(input.getAttribute('data-index'));
            const field = input.getAttribute('data-field');
            if (state.collectionItems[idx]) {
                state.collectionItems[idx][field] = input.value;
                if (field === 'name' || field === 'description') {
                    const infoElem = input.closest('.collection-item-card').querySelector(`.collection-item-${field === 'name' ? 'title' : 'sub'}`);
                    if (infoElem) infoElem.textContent = input.value;
                }
            }
        });
    });
...
async function saveAllCollectionChanges() {
    if (!DOM.btnSaveCollectionAll) return;
    DOM.btnSaveCollectionAll.disabled = true;
    if (DOM.editorStatusMsg) DOM.editorStatusMsg.textContent = 'Salvando alterações...';
    try {
        const response = await fetch('/api/playlists/save-all', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(state.collectionItems)
        });
        const data = await response.json();
        if (response.ok && data.status === 'success') {
            showToast(`Coleção atualizada com sucesso! (${data.count} LPs salvos)`, 'success');
        ...
        if (DOM.editorStatusMsg) DOM.editorStatusMsg.textContent = 'Erro ao salvar.';
    } finally {
        DOM.btnSaveCollectionAll.disabled = false;
    }
}

// Attach Collection Editor Trigger & Controls
if (DOM.btnEditCollection) {
    DOM.btnEditCollection.addEventListener('click', openCollectionEditor);
}
if (DOM.btnCloseCollectionEditor) {
    DOM.btnCloseCollectionEditor.addEventListener('click', closeCollectionEditor);
}
if (DOM.btnCancelCollectionEditor) {
    DOM.btnCancelCollectionEditor.addEventListener('click', closeCollectionEditor);
}
if (DOM.btnSaveCollectionAll) {
    DOM.btnSaveCollectionAll.addEventListener('click', saveAllCollectionChanges);
}
if (DOM.collectionSearchInput) {
    DOM.collectionSearchInput.addEventListener('input', (e) => {
        renderCollectionEditor(e.target.value);
    });
}
